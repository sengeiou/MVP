...
public abstract class BaseAdapter<VB extends ViewBinding, T> extends RecyclerView.Adapter<BaseViewHolder> {
    ...
    public BaseViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        ...
        try {
            ...
            vb.getRoot().setLayoutParams(parent.getLayoutParams());
    ...
    public void onBindViewHolder(@NonNull BaseViewHolder holder, int position) {
        RxView.clicks(holder.itemView).throttleFirst(1, TimeUnit.SECONDS).subscribe(v -> {
            if (onItemClickListener != null) onItemClickListener.onItemClick(position);
        });
        RxView.longClicks(holder.itemView).throttleFirst(1, TimeUnit.SECONDS).subscribe(v -> {
